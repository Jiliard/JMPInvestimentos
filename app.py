...
def obter_dados_base():
    ...
    if _CACHE["df"] is not None and (agora - _CACHE["updated_at"]) < CACHE_TTL:
        print("✅ [CACHE] Retornando dados salvos em memória.")
        return _CACHE["df"].copy()
        
    url = 'https://www.fundamentus.com.br/resultado.php'
    
    # HEADERS AVANÇADOS: Simula um navegador real para evitar bloqueio 403
    headers = {
    ...
    print(f"⏳ [FUNDAMENTUS] Iniciando raspagem de dados...")
    
    try:
        r = requests.get(url, headers=headers, timeout=15)
        
        # LOG DE SUCESSO/FALHA NA CONEXÃO
        if r.status_code != 200:
            print(f"🚨 [ERRO FUNDAMENTUS] Bloqueio detectado! Código de Status: {r.status_code}")
            print(f"🚨 [ERRO FUNDAMENTUS] Resposta do Servidor: {r.text[:300]}")
            return pd.DataFrame()
            
        print("✅ [FUNDAMENTUS] Acesso liberado (Status 200). Lendo tabelas HTML...")
        
        try:
            df_list = pd.read_html(r.text, thousands='.', decimal=',')
            if not df_list:
                print("🚨 [ERRO PANDAS] HTML carregado, mas nenhuma tabela foi encontrada.")
                return pd.DataFrame()
            df = df_list[0]
            print(f"✅ [FUNDAMENTUS] Tabela lida com sucesso! {len(df)} ativos encontrados.")
        except ValueError as ve:
            print(f"🚨 [ERRO PANDAS] Falha ao ler o HTML. O Fundamentus pode ter exigido Captcha: {ve}")
            return pd.DataFrame()
...
def get_rankings():
    df = obter_dados_base()
    if df.empty:
        print("⚠️ [RANKINGS] O DataFrame está vazio. Enviando array vazio para o frontend.")
        return jsonify([])
    ...
    if metodo == "graham":
        ...
        df['potencial'] = np.where(df['preco'] > 0, (df['valor_justo'] - df['preco']) / df['preco'], 0)
        df = df.sort_values(by='potencial', ascending=False)
    elif metodo == "bazin":
        df['preco_teto'] = (df['preco'] * df['dy']) / 0.06
        df['potencial'] = np.where(df['preco'] > 0, (df['preco_teto'] - df['preco']) / df['preco'], 0)
        df = df.sort_values(by='potencial', ascending=False)
...
def get_analise_completa():
    ...
    if df_base.empty:
         return jsonify({"error": "Servidor temporariamente bloqueado na fonte de dados (Fundamentus)."})
    ...
    print(f"⏳ [YFINANCE] Baixando histórico de {ticker_input}.SA...")
    try:
        ...
        if not df_yf.empty:
            ...
            ma50 = series_close.rolling(janela_curta).mean().ffill().bfill().tolist()
            ma200 = series_close.rolling(janela_longa).mean().ffill().bfill().tolist()
            ...
            print(f"✅ [YFINANCE] Histórico de {ticker_input} gerado com sucesso!")
        else:
            print(f"⚠️ [YFINANCE] Nenhuma cotação retornada para {ticker_input}.")
            
    except Exception as e:
        print(f"🚨 [ERRO YFINANCE] Bloqueio ou falha ao baixar {ticker_input}: {e}")
        chart_data = None 
